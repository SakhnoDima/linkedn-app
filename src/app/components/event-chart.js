...
export const EventChart = ({ data }) => {
    ...
    const groupDataByInterval = (events, interval) => {
        ...
        events.forEach((event) => {
            const roundedTime = Math.floor((event.properties.time * 1000) / interval) * interval;
    ...
    const { sendBidData, rejectBidData } = groupDataByInterval(data, timeIntervals[selectedInterval]);
